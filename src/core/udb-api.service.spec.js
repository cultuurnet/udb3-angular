...
describe('Service: UDB3 Api', function () {
  ...
  it('should find offers when provided a query', function (done) {
    var response = {};
    $httpBackend
      .expectGET(baseUrl + 'offers/?q=foo:bar&sort%5Bcreated%5D=desc&start=0&disableDefaultFilters=true&embed=true&workflowStatus=READY_FOR_VALIDATION,APPROVED')
      .respond(JSON.stringify(response));
    service
      .findOffers('foo:bar')
  ...
  it('should find offers when provided no query', function (done) {
    var response = {};
    $httpBackend
      .expectGET(baseUrl + 'offers/?disableDefaultFilters=true&embed=true&sort%5Bcreated%5D=desc&start=0&workflowStatus=READY_FOR_VALIDATION,APPROVED')
      .respond(JSON.stringify(response));
    service
      .findOffers('')
  ...
  it('should find offer to moderate when provided a query', function (done) {
    ...
    var response = {};
    $httpBackend
      .expectGET(baseUrl + 'events/?q=searchquery&audienceType=everyone&availableFrom=' + currentDate + '&availableTo=*&limit=60&start=120&workflowStatus=READY_FOR_VALIDATION')
      .respond(JSON.stringify(response));
    service
